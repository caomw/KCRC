function [ ratio ] = kcrc_pca_recog( dict , dict_label , test , test_label , Tr_Num)
% This function is the KCRC_RLS algorithm (PCA for dimensionality reduction in kernel space).
...
[v,d] = eig(K);
% d2 = sum(d,2);
di = 80; % set the dimension for PCA
B = []; % the matrix for dimensionality reduction in kernel space
for i = 0:di-1
    B = [B,v(:,(size(d,2)-i))];
end
K = B'*K';
...
test3 = []; % initilization
% po=1;
invmat = inv(K'*K+lamda*eye(size(K,2)));
for i = 1:size(test,1)
    % i=5
